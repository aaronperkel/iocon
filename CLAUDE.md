...
- **File uploads in order forms** — `ImageUpload` component renders client-side previews via object URLs. Actual upload is stubbed with a TODO comment; wire to Vercel Blob (already a dependency — the admin gallery upload in `app/api/admin/gallery/route.ts` is the working example) and store the returned URL when ready.
