...
### Stack
Next.js 15 App Router · TypeScript · Tailwind CSS 3 · `next/font/google` (no external font CDN) · `react-colorful` (in-page color picker) · `@vercel/blob` (gallery image storage)
...
```
/                                  app/page.tsx                     ← force-dynamic; hero, about, approved reviews, contact sections
/review                            app/review/page.tsx              ← review form; ?rating=N preselects the crowns (linked from emails)
...
/admin                             app/admin/page.tsx               ← 'use client', gated; tabs: Orders / Email Customers / Reviews / Gallery
...
POST   /api/uploads                app/api/uploads/route.ts         ← public order-form image upload → stored URL
PATCH  /api/orders/[id]            app/api/orders/[id]/route.ts     ← status updates
GET|POST /api/reviews              app/api/reviews/route.ts         ← GET: approved-only unless admin session
PATCH|DELETE /api/reviews/[id]     app/api/reviews/[id]/route.ts    ← admin-only: approve toggle + delete
POST   /api/admin/email            app/api/admin/email/route.ts     ← admin-only custom email send
GET|POST /api/admin/gallery        app/api/admin/gallery/route.ts   ← admin-only gallery list + image upload
DELETE /api/admin/gallery/[id]     app/api/admin/gallery/[id]/route.ts ← admin-only; also deletes the stored image
GET|POST /api/admin/users          app/api/admin/users/route.ts     ← admin-only login-allowlist management
DELETE /api/admin/users/[email]    app/api/admin/users/[email]/route.ts ← revokes immediately; self-removal blocked
...
### Admin auth (`middleware.ts`, `lib/auth.ts`, `lib/admin-users.ts`, `lib/auth-email.ts`)
Email + 6-digit one-time code. `middleware.ts` (matcher `/admin/:path*`, `/api/orders/:path*`, `/api/reviews/:path*`, `/api/admin/:path*`) gates access: no valid session → `/admin` redirects to `/admin/login`; `GET /api/orders`, `PATCH /api/orders/[id]`, `DELETE /api/reviews/[id]`, and everything under `/api/admin` return 401. **`POST /api/orders`, `POST /api/uploads`, `POST /api/contact`, and `GET|POST /api/reviews` are deliberately public** — the order forms, contact form/modal, and `/review` use them from public pages. All public POSTs are hardened (2026-07): per-IP in-memory `rateLimit` (from `lib/auth.ts`; best-effort, per-lambda in prod), `req.json().catch(() => null)` → 400 on bad JSON, type/enum checks, length caps matched to the DB columns (VARCHAR(191) etc.) so bad input 400s instead of failing the INSERT, email-format checks, and CRLF stripping on contact-form fields that reach email headers. The login allowlist lives in the **`admin_users` DB table**, managed from the admin **Admins** tab and seeded by `db:init` with riley@iocongraphics.com + aaron@iocongraphics.com (Aaron's alias on Riley's iCloud+ domain; the allowlist exists for login and Stripe, not for sending, so those addresses appear nowhere in the email code). `lib/admin-users.ts` reads/writes it via **`@tidbcloud/serverless`** (HTTPS) rather than mysql2 because `verifySessionToken` runs in the edge middleware, where TCP sockets aren't available — never import `lib/db.ts` from anything the middleware reaches. Fail-safe: `FALLBACK_ADMIN_EMAILS` (the same two founders) applies when `DATABASE_URL` is absent, the table is empty, or the query fails, so admin access can never be fully lost — and re-running `db:init` re-seeds the founders (the recovery path if one is ever removed by mistake). Self-removal is blocked, so the portal always keeps ≥1 admin.

The admin page has five tabs: **Orders** (filterable list, expandable cards, status buttons, an "Email this customer" shortcut on email-contact orders), **Email Customers** (`components/AdminEmailPortal.tsx` → `POST /api/admin/email`: recipient checklist built from email-contact orders with All / Open orders / None quick-picks, subject + message, two-step inline confirm — no native dialogs), **Reviews** (`components/AdminReviewsPanel.tsx`: moderation queue — approve/hide toggle with a "Shown on site" badge, plus two-step delete; reviews submit unapproved and only approved ones render on the home page), **Gallery** (`components/AdminGalleryPanel.tsx` → `/api/admin/gallery`: Riley uploads a piece with caption + subject tag + product tag + artwork date — her July 2026 ask — plus a list with two-step delete; images >3.5 MB are shrunk client-side before upload — `lib/shrink-image.ts`, shared with the order forms — because Vercel caps request bodies at ~4.5 MB), and **Admins** (`components/AdminUsersPanel.tsx` → `/api/admin/users`: add/remove the emails allowed to sign in, "(you)"-marked, two-step remove). The email API only accepts recipients matching an existing order's contact email (no arbitrary addresses) and sends one email per recipient, sequentially. Status badges use brand scales only (pending=stone, in-progress=gold, completed=olive — same map as `/waitlist`); don't reintroduce the old `blue-*` badges, they don't retheme in dark mode. `app/admin/layout.tsx` carries the metadata (title + noindex) since both admin pages are client components.

**Stateless, no DB** (survives serverless + `next dev`'s per-bundle module copies): request-code mints an HMAC-signed *challenge* cookie holding a hash of (email, code, expiry) — the code itself is never stored — and verify recomputes the hash, then sets an HMAC-signed *session* cookie (30-day, httpOnly). All HMAC is Web Crypto (`crypto.subtle`) so it runs in the edge middleware too. Signing key is **`AUTH_SECRET`** (`.env.local`, gitignored — **mirror into Vercel**; a dev fallback keeps local login working but is insecure; rotating it signs everyone out). Codes expire in 10 min; `verifySessionToken` re-checks the allowlist so removing an email revokes its live sessions. Unknown emails get an identical `{ok:true}` with no cookie (no account enumeration). Rate limiting is in-memory/best-effort (per-lambda only in prod); the 10-min code expiry is the real guard. The sign-in code email lives in `lib/auth-email.ts`, which mirrors `lib/email.ts`'s SMTP transport (keep the two in sync) and no-ops with a console log when SMTP env is absent; it sends from `hello@iocongraphics.com` (the public contact/support address — also what the Stripe support email should be). Every From address must be a registered alias on Riley's iCloud+ custom domain or iCloud rejects the send.
...
/shop
  ...
  ├── Bulk Drawings     → contact modal, subject "Bulk Ordering Inquiry" (>5 drawings — no order form, starts with a conversation)
  ├── Logo              → contact modal, subject "Logo Inquiry"
  └── Graphic           → contact modal, subject "Graphic Inquiry" (tile renamed from "Custom Graphic" per Riley; id stays custom-graphic)
```
...
- `lib/db.ts` — mysql2 pool for **TiDB Cloud Serverless**, configured by `DATABASE_URL` (`mysql://user:pass@host:4000/dbname`, in `.env.local` — mirror into Vercel). Pool is cached on `globalThis` (shared across `next dev` route bundles and warm lambdas), TLS always on. `npm run db:init` (idempotent, `scripts/init-db.mjs`) creates the database + `orders`/`reviews`/`gallery`/`admin_users` tables (seeding the founding admins) and applies post-launch column additions via `ALTER TABLE … ADD COLUMN IF NOT EXISTS` (TiDB syntax; plain MySQL lacks it) — re-run it against the prod `DATABASE_URL` after adding a table or column locally. **When `DATABASE_URL` is absent, orders/reviews/gallery fall back to in-memory arrays** (also `globalThis`-cached) so a fresh clone works — same graceful-degrade pattern as `lib/email.ts`, but state resets on restart.
- `lib/orders.ts` — order forms POST to `/api/orders` → `addOrder()`. The waitlist page awaits `getOrders()` directly as a server component, hiding orders completed more than two weeks ago (`completed_at`, set/cleared by `updateOrderStatus` on status transitions); the admin page fetches via the API and PATCHes `/api/orders/[id]`. All helpers are **async** (DB queries via `lib/db.ts`, snake_case columns mapped to the camelCase `Order` interface; `sharingPlatforms` is a JSON column).
- `lib/order-types.ts` — the `Order` types + label maps, split out so client components can use them. **Client components must never value-import `lib/orders`/`lib/reviews`/`lib/gallery-store`/`lib/db`/`lib/email`** — that drags mysql2/nodemailer into the browser bundle and breaks `next build` (webpack can't compile their `node:` imports; `tsc`/`next dev` won't catch it if the page is never loaded). Those five modules `import 'server-only'` so the mistake now fails the build with a clear error; server code imports from `lib/orders.ts`, which re-exports the shared types.
...
- `lib/reviews.ts` — same async DB pattern, feeding `/review` submissions, the admin Reviews tab, and the home-page Reviews section. Reviews submit with `approved = false`; Riley approves in the admin tab (`PATCH /api/reviews/[id]`) and only then do they render publicly (`getPublicReviews()`, capped at 6 on the home page — the section disappears while none are approved). `GET /api/reviews` checks the session cookie itself: admins get the full moderation queue, everyone else approved-only. The submit form deliberately moved **off** the home page (Aaron, 2026-07): review asks live in email instead — the finished-order alert ends with five crown links and Riley's custom mail carries a smaller footer version, each crown opening `/review?rating=N` with that crown preselected (a GET that only prefills — email link scanners follow URLs, so clicking must never create a review). TODO: verify reviewer made a purchase once auth/order lookup exists.
- `lib/gallery.ts` — client-safe gallery types + labels (`GalleryImage` now carries an optional `date`, shown as "Jul 2026" via `formatGalleryDate`) plus the placeholder `GALLERY_IMAGES` set. Real entries live in **`lib/gallery-store.ts`** (server-only, same DB/in-memory pattern as reviews) and are managed from the admin Gallery tab; `getPublicGalleryImages()` returns the placeholders **only while the store is empty**, so `/gallery` and `/shop` (both `force-dynamic` now — uploads must appear immediately) never render blank. Image files upload to **Vercel Blob** (`BLOB_READ_WRITE_TOKEN` in `.env.local` — create a Blob store in Vercel and mirror the token); without the token, local dev writes to `public/gallery/uploads/` (gitignored) and prod returns 503. `next.config.ts` allowlists `*.public.blob.vercel-storage.com` for `next/image`.
- `lib/email.ts` — all outbound email, via Riley's iCloud+ custom-domain SMTP (nodemailer). Env in `.env.local` (gitignored; mirror into Vercel): `SMTP_HOST`/`SMTP_PORT`/`SMTP_USER`/`SMTP_PASS`, optional `CONTACT_EMAIL_TO` (Riley's inbox, defaults to riley@iocongraphics.com — override to test without emailing her). **The SMTP username must be the account's primary `@icloud.com` address** — the Apple ID (a gmail) gets `550 mailbox does not exist`, the custom-domain addresses get `535`. Three mail kinds: automated customer alerts from `orders@iocongraphics.com` (Reply-To Riley) — order placed (with queue position), moved up in queue (sent to open orders behind a newly completed one), being drawn (`in-progress`), finished (`completed` — ends with the review ask: five crown links, the nth opening `/review?rating=n`; each is the brand crown as `<img>` — `public/brand/crown-email.png`, a PNG because Gmail strips SVG — with gold-styled `alt="♛"` so blocked images degrade to text crowns) — sent **only when `contactMethod === 'email'`**, silent no-op otherwise; custom mail Riley composes in the admin Email tab (`sendCustomEmail`, **from `riley@iocongraphics.com`** — she's the one writing it; same branded shell, greeting "Hi <first name>," and "— Riley" sign-off, plus a smaller review-crowns footer); and mail to Riley (contact form + new-order notifications) with Reply-To the customer. Status-change alerts fire only on real transitions (PATCH compares previous status). Missing SMTP env → helpers log and return instead of throwing, so a fresh clone still works. Email failures never fail the API request (`Promise.allSettled` + log), except the contact form, which returns 502 so the visitor knows to retry.
...
- `ImageUpload.tsx` — controlled multi-image upload with drag-and-drop and preview grid; previews are object URLs, but each `UploadedFile` keeps its `File` (filtered to JPEG/PNG/WebP/GIF) so the forms can upload on submit via `lib/upload-images.ts`
...
- `ReviewForm.tsx` — crown rating (1–5 `CrownMark`s) + text review, POSTs to `/api/reviews`; used only on `/review`, with `initialRating` fed from `?rating=N`
- `InquiryModal.tsx` — native `<dialog>` (showModal: focus containment, Escape, backdrop) wrapping `ContactForm` with a `presetSubject`; opened by the Bulk Drawings / Logo / Graphic shop tiles instead of jumping to `/#contact`, so Riley's email is tagged with what was clicked
...
- `AdminGalleryPanel.tsx` — admin Gallery tab: upload form (image + caption + subject + product + date) and entry list with two-step delete, against `/api/admin/gallery`; shrinks oversized images client-side (canvas, longest edge 2400px) before upload
- `AdminUsersPanel.tsx` — admin Admins tab: login-allowlist management against `/api/admin/users`
...
- **File uploads in order forms** — no longer stubbed: on submit the forms shrink oversized images client-side (`lib/shrink-image.ts`, shared with the admin gallery) and upload each to `POST /api/uploads` via `lib/upload-images.ts` (public endpoint: Blob in prod, `public/order-uploads/` in dev, image-types-only + ~4.5 MB cap + best-effort per-IP rate limit). The stored URLs go into the order `details`, one per line under `Dancer images:` / `Layout images:` / `Inspiration images:`, and the admin Orders tab linkifies them (`linkifyDetails` in `app/admin/page.tsx`). Upload happens before the order POST; if it fails the form shows a distinct error and nothing is submitted.
- **Gallery content** — no longer stubbed: Riley adds real pieces from the admin Gallery tab. The placeholder entries in `lib/gallery.ts` appear only while she hasn't added any.
