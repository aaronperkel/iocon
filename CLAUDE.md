...
```
/                                  app/page.tsx                     ← hero, about, contact sections
...
- `lib/reviews.ts` — same async DB pattern, feeding `/review` submissions and the admin Reviews tab. The submit form deliberately moved **off** the home page (Aaron, 2026-07): review asks live in email instead — the finished-order alert ends with five crown links and Riley's custom mail carries a smaller footer version, each crown opening `/review?rating=N` with that crown preselected (a GET that only prefills — email link scanners follow URLs, so clicking must never create a review). TODO: verify reviewer made a purchase once auth/order lookup exists; reviews still aren't displayed publicly anywhere.
